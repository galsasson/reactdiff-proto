...
void main(){
	...
	float aDiffRate = texture(gradientFieldTex, p).b;
	float bDiffRate = texture(gradientFieldTex, p).b/4;

	vec2 val = texture(tex0, p).rg * 7;
...
//					  bDiffRate * laplacian.y + val.x*val.y*val.y - (killRate+feedRate) * val.y);
	vec2 delta = vec2((killRate * (16.0 - val.x * val.y) + aDiffRate * laplacian.x),
					  (max(-val.y, killRate * (val.x * val.y - val.y - 12.0) + bDiffRate * laplacian.y)));

	fragColor = vec4((val + delta * TIMESTEP) / 7.0, 0.0, 1.0);
